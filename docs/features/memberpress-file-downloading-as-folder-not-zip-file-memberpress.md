# MemberPress File Downloading as Folder, not .ZIP File | MemberPress

## Menu supplémentaire

[](https://memberpress.com/fr/)MemberPress

Le plugin d'adhésion WordPress tout-en-un

 

![abeilles ouvrières](https://memberpress.com/wp-content/themes/memberpress-theme/images/memberpress-logo-color.svg)

				Achetez MemberPress dès aujourd'hui ! Commencez à être payé pour le contenu que vous créez !

[Obtenir MemberPress maintenant](https://memberpress.com/fr/plans/pricing/)

Rechercher

Recherche

# MemberPress Téléchargement du fichier sous forme de dossier et non de fichier .ZIP

Pour que MemberPress soit correctement téléchargé via votre tableau de bord WordPress > page Plugins > section Ajouter un nouveau, il doit s'agir d'un fichier .zip compressé, PAS d'un dossier.

Tous les téléchargements officiels de MemberPress seront téléchargés sous forme de fichier .zip compressé, mais si vous utilisez Safari comme navigateur web, il se peut que vous vous retrouviez avec un dossier, et non avec le fichier .zip compressé. En effet, Safari dispose d'une fonction de décompression automatique qui permet de décompresser automatiquement un fichier .zip lorsque vous le téléchargez.

Pour contourner ce problème, deux possibilités s'offrent à vous :

Chacune d'entre elles est décrite plus en détail ci-dessous :

## Désactiver la fonction de décompression automatique

#### Pour désactiver cette fonction dans Safari (si elle est activée), veuillez suivre les étapes suivantes :

#### Pour désactiver cette fonction dans Chrome (si elle est activée), veuillez suivre les étapes suivantes :

#### Pour désactiver cette fonction dans Firefox (si elle est activée), veuillez suivre les étapes suivantes :

## 

Cet article a-t-il été utile ?

[Oui](https://memberpress.com)
[Non](https://memberpress.com)

### Articles connexes

### Besoin d'aide ?

Vous ne trouvez pas la réponse que vous cherchez ?[Support de contact](https://memberpress.com)### Contenu

 

### Articles utiles

 

 

![fille de l'ordinateur](data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0MDkiIGhlaWdodD0iMzY0IiB2aWV3Qm94PSIwIDAgNDA5IDM2NCI+PHJlY3Qgd2lkdGg9IjEwMCUiIGhlaWdodD0iMTAwJSIgc3R5bGU9ImZpbGw6I2NmZDRkYjtmaWxsLW9wYWNpdHk6IDAuMTsiLz48L3N2Zz4=)![fille de l'ordinateur](https://memberpress.com/wp-content/themes/memberpress-theme/images/FooterCTAgirl.webp)

### Achetez MemberPress dès aujourd'hui !

Commencez à être payé pour le contenu que vous créez.

[Obtenir MemberPress maintenant](https://memberpress.com/fr/plans/pricing/)

      Recommandé par les meilleurs influenceurs et hébergeurs
			

![4,8 sur 5 sur Capterra](data:image/svg+xml;base64,)![4,8 sur 5 sur Capterra](https://memberpress.com/wp-content/themes/memberpress-theme/images/capterra-footer.png)

![4,7 sur 5 sur G2](data:image/svg+xml;base64,)![4,7 sur 5 sur G2](https://memberpress.com/wp-content/themes/memberpress-theme/images/g2-footer.png)

![4,8 sur 5 sur Trust Pilot](data:image/svg+xml;base64,)![4,8 sur 5 sur Trust Pilot](https://memberpress.com/wp-content/themes/memberpress-theme/images/trustpilot-footer.png)

[](https://www.wpbeginner.com/plugins/5-best-wordpress-membership-plugins-compared/)

![Administration la plus facile - Été 2024](data:image/svg+xml;base64,)![Administration la plus facile - Été 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/easiest-admin.png)

![L'installation la plus facile - Été 2024](data:image/svg+xml;base64,)![L'installation la plus facile - Été 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/easiest-setup.png)

![Haut niveau de performance - Été 2024](data:image/svg+xml;base64,)![Haut niveau de performance - Été 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/high-performer.png)

![Leader de l'élan - Été 2024](data:image/svg+xml;base64,)![Leader de l'élan - Été 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/momentum-leader.png)

## Pied de page

[FR](https://memberpress.com)

[FR](https://memberpress.com)
[EN](https://memberpress.com/docs/memberpress-file-folder-not-zip-file/)
[ES](https://memberpress.com/es/docs/memberpress-file-folder-not-zip-file/)
[DE](https://memberpress.com/de/docs/memberpress-file-folder-not-zip-file/)
[PT](https://memberpress.com/pt/docs/memberpress-file-folder-not-zip-file/)

#### Entreprise

#### Caractéristiques

#### Intégrations

#### Études de cas

#### Liens utiles

 

![Logo de MemberPress](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)![Logo de MemberPress](https://memberpress.com/wp-content/themes/memberpress-theme/images/memberpress-logo-white.svg)

[](https://twitter.com/memberpress)
[](https://facebook.com/memberpress)
[](https://www.instagram.com/memberpress)
[](https://www.youtube.com/c/MemberPressPlugin)

						Copyright © 2025 Caseproof, LLC. Tous droits réservés.
					

[Politique de confidentialité](https://memberpress.com/fr/privacy/)
/
[Remboursements](https://memberpress.com/fr/terms/#refunds)
/
[Conditions générales d'utilisation](https://memberpress.com/fr/terms/)
/
[Divulgation de la FTC](https://memberpress.com/fr/privacy/)
/
[Code promo MemberPress](https://memberpress.com/fr/memberpress-coupon-code/)

 

Nom d'utilisateur ou adresse de messagerie.

Mot de passe

 Se souvenir de moi

 

 

[Mot de passe oublié](https://memberpress.com/fr/sign-in/?action=forgot_password)

			×			

 

  ![](https://px.ads.linkedin.com/collect/?pid=3219420&fmt=gif)

