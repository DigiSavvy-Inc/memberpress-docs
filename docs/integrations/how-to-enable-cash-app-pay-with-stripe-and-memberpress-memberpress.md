# How to Enable Cash App Pay With Stripe and MemberPress? | MemberPress

## Additional menu

[](https://memberpress.com/)MemberPress

The All-In-One WordPress Membership Plugin

 

![worker bees](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)![worker bees](https://memberpress.com/wp-content/themes/memberpress-theme/images/memberpress-logo-color.svg)

				Get MemberPress today! Start getting paid for the content you create!

[Get MemberPress Now](https://memberpress.com/plans/pricing/)

Search For

Search

# How to Enable Cash App Pay With Stripe and MemberPress?

Cash App Pay is an eWallet-based payment method available in the United States. It allows customers to save their debit and credit card details and authorize payments through the platform.

MemberPress integration with Stripe allows you to receive payments through Cash App Pay. This document will show you how to set this up and provide more details on Cash App Pay.

## Receiving Payments With Cash App Pay

Cash App Pay is supported only through the built-in MemberPress integration with Stripe. Thus, before enabling Cash App Pay, please connect Stripe to MemberPress on your site, and test the connection to make sure it’s functioning well.

After your Stripe connection is properly set, please enable Cash App Pay as a payment method in your Stripe account, and then in MemberPress. You will find the Cash App Pay option in your Stripe account by navigating to Settings > Payment Methods and scrolling down to the “Wallets” section.

[](https://memberpress.com/wp-content/uploads/2024/02/file-8nRca4hBGK-1.png)
Please check more details on setting this up in the “How To Enable Payment Methods With Stripe and Memberpress” document.

Note: Your Stripe payment form will show Cash App Pay as a payment method when you toggle on the Cash App Pay option in MemberPress settings. Please keep in mind that Cash App Pay will be available on your forms, even if it's not enabled as a payment method in your Stripe account. To avoid any possible issues, please make sure that Cash App Pay is enabled in your Stripe account before enabling it in MemberPress.

Once Cash App Pay is enabled, your membership registration forms will show this new payment option. The Cash App Pay payment option will be available on both Stripe Checkout and Stripe Element forms.

When your users submit the registration form, they will be redirected to Cash App Pay to authorize the payment and returned to your site once the payment is confirmed.

## What’s Important to Know About Using Cash App Pay?

Cash App Pay can be used as a payment method for both recurring and non-recurring payments. Also, Cash App Pay supports memberships with free or paid trials.

On the other hand, the Cash App Pay payment option will show only if all conditions are met, as Cash App Pay integration comes with several limitations imposed by Cash App Pay or Stripe.

Further, Cash App Pay supports only the US Dollar as a currency. Thus, to use Cash App Pay with MemberPress, please make sure that the currency in MemberPress settings is set to USD.

In addition, both the country selected for your Stripe account and the country set in your MemberPress settings must be the United States. Also, if you enabled MemberPress address fields on your registration forms, the Cash App Pay payment method will be visible only if the user’s country is set to the United States.

You can also find more information about Cash App Pay payments in Stripe documentation.

Was this article helpful?

[Yes](https://memberpress.com)
[No](https://memberpress.com)

### Related Articles

### Need Support?

Can't find the answer you're looking for?[Contact Support](https://memberpress.com)### Contents

 

### Helpful Articles

 

 

![computer girl](data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI0MDkiIGhlaWdodD0iMzY0IiB2aWV3Qm94PSIwIDAgNDA5IDM2NCI+PHJlY3Qgd2lkdGg9IjEwMCUiIGhlaWdodD0iMTAwJSIgc3R5bGU9ImZpbGw6I2NmZDRkYjtmaWxsLW9wYWNpdHk6IDAuMTsiLz48L3N2Zz4=)![computer girl](https://memberpress.com/wp-content/themes/memberpress-theme/images/FooterCTAgirl.webp)

### Get MemberPress today!

Start getting paid for the content you create.

[Get MemberPress Now](https://memberpress.com/plans/pricing/)

      Recommended by top influencers and web hosts
			

![4.8 out of 5 on Capterra](data:image/svg+xml;base64,)![4.8 out of 5 on Capterra](https://memberpress.com/wp-content/themes/memberpress-theme/images/capterra-footer.png)

![4.7 out of 5 on G2](data:image/svg+xml;base64,)![4.7 out of 5 on G2](https://memberpress.com/wp-content/themes/memberpress-theme/images/g2-footer.png)

![4.8 out of 5 on Trust Pilot](data:image/svg+xml;base64,)![4.8 out of 5 on Trust Pilot](https://memberpress.com/wp-content/themes/memberpress-theme/images/trustpilot-footer.png)

[](https://www.wpbeginner.com/plugins/5-best-wordpress-membership-plugins-compared/)

![Easiest Admin - Summer 2024](data:image/svg+xml;base64,)![Easiest Admin - Summer 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/easiest-admin.png)

![Easiest Setup - Summer 2024](data:image/svg+xml;base64,)![Easiest Setup - Summer 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/easiest-setup.png)

![High Performer - Summer 2024](data:image/svg+xml;base64,)![High Performer - Summer 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/high-performer.png)

![Momentum Leader - Summer 2024](data:image/svg+xml;base64,)![Momentum Leader - Summer 2024](https://memberpress.com/wp-content/themes/memberpress-theme/images/verification-badges/momentum-leader.png)

## Footer

 

#### Company

#### Features

#### Integrations

#### Case Studies

#### Helpful Links

 

![MemberPress logo](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)![MemberPress logo](https://memberpress.com/wp-content/themes/memberpress-theme/images/memberpress-logo-white.svg)

[](https://twitter.com/memberpress)
[](https://facebook.com/memberpress)
[](https://www.instagram.com/memberpress)
[](https://www.youtube.com/c/MemberPressPlugin)

						Copyright © 2025 Caseproof, LLC. All rights reserved.
					

[Privacy Policy](https://memberpress.com/privacy/)
/
[Refunds](https://memberpress.com/terms/#refunds)
/
[Terms & Conditions](https://memberpress.com/terms/)
/
[FTC Disclosure](https://memberpress.com/privacy/)
/
[MemberPress Coupon Code](https://memberpress.com/memberpress-coupon-code/)

 

Username or E-mail

Password

 Remember Me

 

 

[Forgot Password](https://memberpress.com/sign-in/?action=forgot_password)

			×			

 

  ![](https://px.ads.linkedin.com/collect/?pid=3219420&fmt=gif)

